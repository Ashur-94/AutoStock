...
export const InvoiceUploadModal: React.FC<InvoiceUploadModalProps> = ({
  ...
  // AI Invoice Scanner API Call
  const handleScanInvoiceWithAI = async () => {
    if (!selectedImage) return;

    setIsProcessing(true);
    setErrorMsg(null);
    setProcessingStep('جاري الاتصال بمحرك الذكاء الاصطناعي Gemini...');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6 bg-slate-950/80 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-200" dir="rtl">
      <div className="relative w-full max-w-[calc(100vw-1rem)] sm:max-w-4xl bg-slate-900 border border-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0 text-right">
              <h2 className="text-sm sm:text-lg font-bold text-white flex items-center gap-1.5 sm:gap-2">
                ...
                <span className="text-[10px] sm:text-xs font-semibold px-1.5 sm:px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30 shrink-0">
                  بالذكاء الاصطناعي
                </span>
              </h2>
        ...
        <div className="p-3.5 sm:p-6 overflow-y-auto flex-1 space-y-4 sm:space-y-6 text-right">
          ...
          {/* Navigation Tabs */}
          {!parsedResult && (
            <div className="flex items-center gap-1 sm:gap-2 bg-slate-950 p-1 rounded-xl border border-slate-800 overflow-x-auto w-full">
              <button
            ...
            <div>
              {selectedImage ? (
                <div className="space-y-4">
                  <div className="relative rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden max-h-72 flex items-center justify-center p-2">
                    <img
                      src={selectedImage}
                      alt="معاينة الفاتورة"
                      className="max-h-68 rounded-xl object-contain shadow-md"
                    />
                    <button
                      onClick={() => setSelectedImage(null)}
                      className="absolute top-4 left-4 bg-slate-900/80 hover:bg-slate-900 text-white p-2 rounded-xl border border-slate-700 text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
                    >
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-bold text-sm flex items-center justify-center gap-2 shadow-xl shadow-amber-500/20 transition-all cursor-pointer disabled:opacity-50"
                  >
                    {isProcessing ? (
                      <>
                ...
                <div
                  ...
                  className="border-2 border-dashed border-slate-700 hover:border-amber-500/70 bg-slate-950/60 hover:bg-slate-950 rounded-2xl p-8 sm:p-12 text-center cursor-pointer transition-all flex flex-col items-center justify-center gap-3"
                >
                  <input
                    ref={fileInputRef}
                    type="file"
                  ...
                  <button
                    type="button"
                    className="mt-2 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-300 font-semibold text-xs border border-slate-700 cursor-pointer"
                  >
          ...
          {activeTab === 'camera' && !parsedResult && (
            <div className="space-y-4">
              <div className="relative rounded-2xl bg-black border border-slate-800 overflow-hidden aspect-video max-h-80 flex items-center justify-center">
                <video
                  ref={videoRef}
                  autoPlay
                  playsInline
                ...
                {/* Guide overlay box */}
                <div className="absolute inset-8 border-2 border-dashed border-amber-400/50 rounded-xl pointer-events-none flex items-center justify-center">
                  <span className="text-[11px] font-semibold text-amber-300/80 bg-slate-950/70 px-3 py-1 rounded-full">
              ...
              <div className="flex items-center justify-center gap-3">
                <button
                  ...
                  className="px-6 py-3 rounded-2xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-sm flex items-center gap-2 shadow-lg shadow-amber-500/20 cursor-pointer"
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {SAMPLE_INVOICE_PRESETS.map((preset) => (
                  <div
                    ...
                    className="p-4 rounded-2xl bg-slate-950 border border-slate-800 hover:border-amber-500/50 hover:bg-slate-900 transition-all cursor-pointer flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-mono font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20" dir="ltr">
          ...
          {parsedResult && (
            <div className="space-y-4 sm:space-y-5 animate-in fade-in duration-300">
              ...
              <div className="p-3.5 sm:p-4 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 w-full min-w-0">
                <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                ...
                <div className="flex items-center justify-between w-full sm:w-auto gap-2 sm:gap-4 text-xs font-mono pt-2 sm:pt-0 border-t border-slate-800 sm:border-0">
                  <div>
                    <span className="text-[9px] sm:text-[10px] text-slate-500 block">رقم الفاتورة</span>
                    <span className="text-amber-300 font-bold text-[11px] sm:text-xs" dir="ltr">{parsedResult.invoiceNumber}</span>
                  </div>
                  <div>
                    <span className="text-[9px] sm:text-[10px] text-slate-500 block">التاريخ</span>
                    <span className="text-slate-300 text-[11px] sm:text-xs" dir="ltr">{parsedResult.invoiceDate}</span>
                  </div>
                  <div>
                    <span className="text-[9px] sm:text-[10px] text-slate-500 block">الإجمالي</span>
                    <span className="text-emerald-400 font-bold text-xs sm:text-sm" dir="ltr">
                ...
                <div className="space-y-2 max-h-72 sm:max-h-80 overflow-y-auto pl-0.5">
                  {parsedResult.items.map((item, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className="p-3 sm:p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 sm:gap-3 text-xs w-full min-w-0"
                      >
                        {/* Part Info */}
                        <div className="flex-1 min-w-0 w-full sm:w-auto text-right">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="font-mono font-bold text-[10px] sm:text-[11px] px-1.5 py-0.5 rounded bg-slate-900 text-amber-300 border border-slate-800" dir="ltr">
                              {item.partNumber}
                            </span>
                            <span className="text-[9px] sm:text-[10px] font-semibold px-1.5 py-0.5 rounded bg-slate-800 text-slate-300">
                              {normalizeCategory(item.category)}
                            </span>
                          </div>

                          <div className="font-bold text-slate-100 text-xs sm:text-sm mt-1 truncate">
                            {item.name}
                          </div>
                          ...
                          <div className="mt-1 text-[10px] sm:text-[11px]">
                            {matchedStock ? (
                              <span className="text-emerald-400 font-medium flex items-center gap-1">
                                ...
                                <span>مطابق في الورشة: المتوفر {matchedStock.quantity} ➔ <strong className="text-emerald-300 font-mono">{newTotalQty} بعد التوريد</strong></span>
                              </span>
                            ) : (
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-2.5 w-full sm:w-auto pt-2 sm:pt-0 border-t border-slate-800/80 sm:border-0 shrink-0">
                          <div>
                            ...
                            <div className="flex items-center gap-1 mt-0.5">
                              <input
                                ...
                                className="w-14 sm:w-16 font-mono font-extrabold text-xs sm:text-sm text-center bg-slate-800 text-emerald-400 border border-slate-700 rounded-lg py-1 focus:outline-none focus:border-emerald-500"
                              />
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-0.5 mt-0.5 font-mono" dir="ltr">
                              ...
                              <input
                                ...
                                className="w-16 sm:w-20 font-mono font-semibold text-xs sm:text-sm text-center bg-slate-800 text-slate-200 border border-slate-700 rounded-lg py-1 focus:outline-none"
                              />
                            </div>
                          </div>
                          ...
                          <button
                            ...
                            className="text-slate-500 hover:text-rose-400 p-1.5 rounded hover:bg-slate-800 transition-colors mt-3 sm:mt-0 shrink-0 cursor-pointer"
                            title="حذف هذا البند"
                          >
                            <X className="w-4 h-4" />
                          </button>
        ...
        {/* Modal Footer */}
        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-slate-900 border-t border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          {parsedResult ? (
            <>
              <button
                onClick={() => {
                ...
                className="order-2 sm:order-1 px-3 py-2 rounded-xl text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800 transition-colors text-center cursor-pointer"
              >
                مسح فاتورة أخرى
              </button>
              ...
              <button
                ...
                className="order-1 sm:order-2 px-4 sm:px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 cursor-pointer"
              >
            ...
            <div className="w-full flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-xl text-xs font-semibold text-slate-400 hover:text-white hover:bg-slate-800 transition-colors shrink-0 cursor-pointer"
              >
                إلغاء
              </button>
            </div>
