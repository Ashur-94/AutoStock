...
export const CounterSaleBar: React.FC<CounterSaleBarProps> = ({
  ...
  if (sessionSales.length === 0) {
    return (
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 bg-white border border-amber-400 text-slate-900 px-3.5 sm:px-5 py-2.5 sm:py-3 rounded-2xl shadow-xl backdrop-blur-md flex items-center justify-between gap-2.5 max-w-[calc(100vw-1.5rem)] w-[94vw] sm:w-auto animate-in slide-in-from-bottom duration-200" dir="rtl">
        <div className="flex items-center gap-2 min-w-0">
          <span className="w-2 h-2 rounded-full bg-amber-500 animate-ping shrink-0" />
          <span className="text-[11px] sm:text-xs font-bold text-amber-800 truncate">
            وضع البيع السريع نشط: اضغط <span className="font-mono text-slate-900 bg-rose-100 px-1 py-0.5 rounded border border-rose-300">-</span> على أي قطعة لخصمها
          </span>
        </div>
        <button
          onClick={onCloseQuickSale}
          className="shrink-0 text-xs font-semibold px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors cursor-pointer"
        >
          تم
        </button>
      </div>
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-5 left-1/2 -translate-x-1/2 z-40 bg-white border border-amber-500 text-slate-900 px-3.5 sm:px-6 py-2.5 sm:py-3 rounded-2xl shadow-xl shadow-amber-500/10 backdrop-blur-md flex items-center gap-2.5 sm:gap-4 max-w-[calc(100vw-1.5rem)] w-[94vw] sm:w-auto animate-in slide-in-from-bottom duration-200" dir="rtl">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <button
          ...
          className="p-1.5 rounded-lg text-slate-500 hover:text-rose-600 hover:bg-slate-100 transition-colors text-xs cursor-pointer"
        ...
        <button
          onClick={onCloseQuickSale}
          className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs flex items-center gap-1 transition-all whitespace-nowrap cursor-pointer shadow-sm"
        >
